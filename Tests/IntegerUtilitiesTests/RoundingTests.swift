...
final class IntegerUtilitiesRoundingTests: XCTestCase {
  ...
  func testRoundingNearest<T: BinaryFloatingPoint>(_ type: T.Type) {
    ...
    for vector in vectors {
      ...
      XCTAssertEqual(
        vector.input.rounding(.toNearestOrDown), vector.down,
        "\(vector.input).rounding(.toNearestOrDown)")
      if vector.down == 0 {
        XCTAssertEqual(
          vector.input.rounding(.toNearestOrDown).sign, vector.input.sign,
          "\(vector.input).rounding(.toNearestOrDown)")
      }

      XCTAssertEqual(
        vector.input.rounding(.toNearestOrUp), vector.up, "\(vector.input).rounding(.toNearestOrUp)"
      )
      if vector.up == 0 {
        XCTAssertEqual(
          vector.input.rounding(.toNearestOrUp).sign, vector.input.sign,
          "\(vector.input).rounding(.toNearestOrUp)")
      }

      XCTAssertEqual(
        vector.input.rounding(.toNearestOrZero), vector.zero,
        "\(vector.input).rounding(.toNearestOrZero)")
      if vector.zero == 0 {
        XCTAssertEqual(
          vector.input.rounding(.toNearestOrZero).sign, vector.input.sign,
          "\(vector.input).rounding(.toNearestOrZero)")
      }

      XCTAssertEqual(
        vector.input.rounding(.toNearestOrAway), vector.away,
        "\(vector.input).rounding(.toNearestOrAway)")
      if vector.away == 0 {
        XCTAssertEqual(
          vector.input.rounding(.toNearestOrAway).sign, vector.input.sign,
          "\(vector.input).rounding(.toNearestOrAway)")
      }

      XCTAssertEqual(
        vector.input.rounding(.toNearestOrEven), vector.even,
        "\(vector.input).rounding(.toNearestOrEven)")
      if vector.even == 0 {
        XCTAssertEqual(
          vector.input.rounding(.toNearestOrEven).sign, vector.input.sign,
          "\(vector.input).rounding(.toNearestOrEven)")
      }
    }
  }
  ...
  func testRoundingOdd<T: BinaryFloatingPoint>(_ type: T.Type) {
    ...
    for vector in vectors {
      ...
      if vector.odd == 0 {
        XCTAssertEqual(
          vector.input.rounding(.toOdd).sign, vector.input.sign, "\(vector.input).rounding(.toOdd)")
      }
    }
  }
